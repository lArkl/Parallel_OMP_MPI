 ...
 *
 * Compile:  mpicc -g -Wall prog_fuente.c -o prog_ejecutable
...
void Create_digraph(int n_cities);
...
int main(int argc, char* argv[]) {
   ...
   MPI_Comm_rank(comm, &my_rank);
	int n_cities;
	for(n_cities=5; n_cities<16; n_cities+=5){
		/*
 ...
 * Purpose:   Creates the digraph of costs for the cities
 * In arg:    None
 * Globals out:
...
void Create_digraph(int n_cities) {
   ...
   if (my_rank == 0) {
      for (i = 0; i < n; i++)
         for (j = 0; j < n; j++) {
            digraph[i*n + j] = i==j? 0: rand()%30+1; // always has a cost unless it's the same node
         }
   }
