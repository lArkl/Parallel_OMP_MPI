/* Modelo de problema de solucion de ecuacion de calor
 * examen parcial Algoritmos Paralelos - p.3
 * Prof. Jose Fiestas
 * */
...
int main ( int argc, char *argv[] )
{
	...
	double temp[n],tempnew[n],x[n+2],dx;
	...
	*/
	...
	for ( j = 1; j <= j_max; j++ ){
		...
		*/
		for ( i = 1; i <= n; i++ ){
			tempnew[i] = temp[i] 
			+ ( dt * k / dx / dx ) * ( temp[i-1] - 2.0 * temp[i] + temp[i+1] ); 
		}
	...
	*/
		if ( 0 == rank )   
			tempnew[1] = frontera( x[1], tnew );
		if ( rank == p - 1 )  
			tempnew[n] = frontera( x[n], tnew );
	...
	*/
		...
		int serie = (j%100!=0)?-1:j/100;
		//if (j==1)serie=1;
		if(serie!=-1){
			sprintf (buffer, "%01d.dat",serie);
			FILE *fp;
			fp = fopen(buffer, "a");
			printf("%d\n",serie);
			if (fp==NULL){
